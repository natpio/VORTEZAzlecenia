...
st.markdown("Rejestracja kosztów sprowadzenia i zwrotu sprzętu pod konkretne ID Projektu.")
...
with st.form("form_zaopatrzenie"):
    ...
    opis = st.text_area("Co dokładnie jedzie? (Np. 4x Głośnik, 2x Statyw)")
    
    if st.form_submit_button("✅ Zapisz Koszt Zaopatrzenia"):
        nr_ref = f"ZAOP/{id_p}/{datetime.now().strftime('%d%m')}"
        # Zapisujemy do zakładki Zlecenia: P = ID Projektu, Q = Typ (Inbound/Zwrot), R = Stawka
        nowy_wiersz = [datetime.now().strftime("%Y-%m-%d %H:%M"), nr_ref, "ZAOPATRZENIE", "", kontrahent, "NASZ MAGAZYN", "", "", "Sprzęt Wypożyczony", "", "", "", "", opis, "", id_p, typ_ruchu, stawka]
        sh.worksheet("Zlecenia").append_row(nowy_wiersz)
        st.success(f"Zapisano koszt {typ_ruchu} dla projektu {id_p}")
